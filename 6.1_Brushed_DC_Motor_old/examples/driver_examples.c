...
static uint8_t example_EDBG_UART[12] = "Hello World!";

static void tx_cb_EDBG_UART(const struct usart_async_descriptor *const io_descr)
{
	/* Transfer completed */
}
...
void EDBG_UART_example(void)
{
	...
	usart_async_register_callback(&EDBG_UART, USART_ASYNC_TXC_CB, tx_cb_EDBG_UART);
	/*usart_async_register_callback(&EDBG_UART, USART_ASYNC_RXC_CB, rx_cb);
	usart_async_register_callback(&EDBG_UART, USART_ASYNC_ERROR_CB, err_cb);*/
	usart_async_get_io_descriptor(&EDBG_UART, &io);
	usart_async_enable(&EDBG_UART);

	io_write(io, example_EDBG_UART, 12);
}
